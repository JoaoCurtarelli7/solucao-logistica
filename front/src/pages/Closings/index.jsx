...
export default function Closings() {
  ...
  useEffect(() => {
    if (!canViewClosings) return;
    loadMonths();
    loadCompanies();
  }, [canViewClosings]);

  useEffect(() => {
    if (!canViewClosings || !selectedMonth) return;
    loadClosings();
  }, [selectedMonth, canViewClosings]);
    ...
    try {
      ...
      const submitData = {
        ...
        startDate: resolved.startDate
          ? resolved.startDate.format("DD/MM/YYYY")
          : null,
        endDate: resolved.endDate
          ? resolved.endDate.format("DD/MM/YYYY")
          : null,
      };
      ...
      message.error(
        error.response?.data?.message || "Erro ao criar fechamento",
      );
    }
  };
  ...
  const handleDeleteClosing = async (id) => {
    try {
      ...
      message.error(
        error.response?.data?.message || "Erro ao deletar fechamento",
      );
    }
  };
  ...
  const handleEdit = (closing) => {
    ...
    if (startDate && endDate) {
      ...
      if (
        lowerName.includes("1ª quinzena") ||
        lowerName.includes("1a quinzena")
      ) {
        closingType = "first_half";
      } else if (
        lowerName.includes("2ª quinzena") ||
        lowerName.includes("2a quinzena")
      ) {
        closingType = "second_half";
    ...
    form.setFieldsValue({
      ...
      monthPicker:
        closingType !== "custom" && startDate ? startDate : undefined,
    });
    setIsModalOpen(true);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "right",
      render: (value) => (
        <Text style={{ color: "#52c41a" }}>{formatCurrency(value || 0)}</Text>
      ),
    },
    {
      title: "Saídas",
      ...
      align: "right",
      render: (value) => (
        <Text style={{ color: "#ff4d4f" }}>{formatCurrency(value || 0)}</Text>
      ),
    },
    {
      title: "Impostos",
      ...
      align: "right",
      render: (value) => (
        <Text style={{ color: "#faad14" }}>{formatCurrency(value || 0)}</Text>
      ),
    },
    {
      title: "Saldo",
              ...
              <Button
                ...
                title={
                  record.status === "fechado"
                    ? "Não é possível deletar fechamento fechado"
                    : "Deletar"
                }
              />
            </Popconfirm>
              ...
              <Col span={6}>
                <Card>
                  <Statistic
                    title="Fechados"
                    value={
                      closings.filter((c) => c.status === "fechado").length
                    }
            ...
            <Table
              ...
              pagination={{
                ...
                showTotal: (total, range) =>
                  `${range[0]}-${range[1]} de ${total} fechamentos`,
              }}
            />
          </>
        )}
      ...
      {(canCreateClosings || canUpdateClosings) && (
        ...
        >
          <Form
            ...
            layout="vertical"
            onFinish={
              editingClosing ? handleUpdateClosing : handleCreateClosing
            }
          >
            <Form.Item
                  ...
                  return (
                    <>
                      ...
                      <Form.Item
                        ...
                        name="endDate"
                        rules={[
                          { required: true, message: "Data fim é obrigatória" },
                        ]}
                      >
                        <DatePicker
                          style={{ width: "100%" }}
