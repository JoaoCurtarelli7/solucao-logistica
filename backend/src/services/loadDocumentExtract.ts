...
function findLabelValue(
  ...
  for (
    let i = index + 1;
    i < Math.min(lines.length, index + 1 + lookAhead);
    i += 1
  ) {
    const line = lines[i];
    const match = line.match(valuePattern);
    if (match?.[1]) return match[1].trim();
    if (valuePattern.test(line)) return line.trim();
  }
  return null;
}

function findWeightKg(lines: string[]): number | null {
  const index = lines.findIndex((line) =>
    /Peso Total do Carregamento/i.test(line),
  );
  if (index === -1) return null;
  ...
  for (let i = index + 1; i < Math.min(lines.length, index + 16); i += 1) {
    ...
    if (inline?.[1]) return parseBrNumber(inline[1]);
    const numeric = parseBrNumber(line);
    if (numeric != null && numeric > 0) candidates.push(numeric);
...
function findWeightFromText(text: string): number | null {
  return (
    parseBrNumber(matchFirst(text, [/([\d.,]+)\s*Kg/i, /([\d.,]+)\nKg/i])) ||
    null
  );
}
...
function findMoneyAfterLabel(
  ...
  for (
    ...
    if (value != null && value > 0) best = value;
  }
  return best;
}

function findSummaryTotal(lines: string[]): number | null {
  const totalIndex = lines.findIndex((line) => /^Total$/i.test(line));
  if (totalIndex === -1) return null;
  for (
    let i = totalIndex + 1;
    i < Math.min(lines.length, totalIndex + 8);
    i += 1
  ) {
    const value = parseBrNumber(lines[i]);
...
export function suggestLoadFallback(params: {
  pdfText: string;
  companies: { id: number; name: string; cnpj: string }[];
}): SuggestedLoadFromDocument {
  const text = params.pdfText;
  const lines = getNonEmptyLines(text);

  const loadingNumber =
    findLabelValue(lines, /Resumo do Carregamento/i, /^(\d{4,})$/) ||
    matchFirst(text, [
  ...
  const date =
    findLabelValue(lines, /Data sa[íi]da/i, /(\d{2}\/\d{2}\/\d{4})/) ||
    matchFirst(text, [
  ...
  const deliveriesRaw =
    findLabelValue(lines, /Qtde de Entregas/i, /^(\d+)$/) ||
  ...
  const totalValue =
    findMoneyAfterLabel(lines, /Vl\.\s*L[íi]quido a Receber/i) ||
  ...
  if (loadingNumber) filledFields.push("loadingNumber");
  else missingImportant.push("número do carregamento");

  if (date) filledFields.push("date");
  else missingImportant.push("data da carga");

  if (deliveries != null) filledFields.push("deliveries");
  else missingImportant.push("quantidade de entregas");

  if (cargoWeight != null) filledFields.push("cargoWeight");
  else missingImportant.push("peso da carga");

  if (totalValue != null) filledFields.push("totalValue");
  else missingImportant.push("valor total");

  if (observations) filledFields.push("observations");
...
export async function suggestLoadWithOpenAI(params: {
  pdfText: string;
  companies: { id: number; name: string; cnpj: string }[];
}): Promise<SuggestedLoadFromDocument> {
  ...
  const truncated = params.pdfText.slice(0, 14_000);
...
export function mergeCompanyMatch(
  suggestion: SuggestedLoadFromDocument,
  companies: { id: number; cnpj: string }[],
): number | null {
  const ids = new Set(companies.map((c) => c.id));
  if (
    suggestion.matchedCompanyId != null &&
